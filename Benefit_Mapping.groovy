...
def void mapBnefAliment(Message message, String obj, String body, String dateFormat) {
    ...
    if(jsonObject.option.data != null && jsonObject.option.data != "null"){   // Testa se candidato optou pelo benefício
...
//------------------------------------------------------------------------------------------------------------------------------------
    if(jsonObj_body.pagamento.vinculo != "aprendiz" && jsonObj_body.pagamento.vinculo != "estagio"){ 
    ...
    //------------------------------------------------------------------------------------------------------------------------------------
        if(jsonObject.option.data.incluirDependentes && dep_lenght != 0){ //-- Incluir dependentes?
            ...
            if(jsonObject.option.data.listaDependentes != null){ // Verifica se existe uma  lista de dependentes
                
                if(jsonObject.option.data.listaDependentes.size() != 0){ // Verifica se a dependentes para esse beneficio
            
                    jsonObject.option.data.listaDependentes.forEach {e -> 
                    
                        if(e.elegivel){ //-- O dependente está elegível?
...
//------------------------------------------------------------------------------------------------------------------------------------
    if(jsonObject.option.data.incluirBeneficiarios){ //-- Incluir beneficiários?
        ...
        jsonObject.option.data.listaDependentes.forEach {e -> 
            ...
            if(e.elegivel){ //-- O beneficiário está elegível?
